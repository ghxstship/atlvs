...
const navItemVariants = cva(
  [
    'flex items-center gap-sm px-sm py-sm mx-2 rounded-lg text-sm font-medium',
...
export const SidebarNavigation: React.FC<SidebarNavigationProps> = ({
    ...
    return (
      <div key={item.id} className="relative">
        ...
        >
          ...
          {isCollapsed && (
            <div className="absolute left-full ml-2 px-sm py-xs bg-popover text-popover-foreground text-xs rounded opacity-0 group-hover:opacity-100 motion-safe:transition-opacity motion-safe:duration-200 pointer-events-none whitespace-nowrap z-50">
              {item.label}
  ...
  if (isMobile && variant === 'overlay') {
    return (
      <>
        ...
        <button
          className="lg:hidden fixed top-4 left-4 z-50 p-sm bg-background border border-border rounded-lg shadow-sm"
              ...
              <div className="p-md">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-sm bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                  />
                </div>
              </div>
              
              {/* Navigation */}
              <nav className="flex-1 overflow-y-auto px-sm pb-4">
  ...
  return (
    <div aria-label="Sidebar Navigation" className={twMerge(sidebarVariants({ 
        ...
        <div className="p-md">
          <div className="relative">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-sm bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary motion-safe:transition-all motion-safe:duration-200"
              onKeyDown={(e) => {
                if (e.key === 'Escape') {
                  setSearchQuery('');
      ...
      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto px-sm pb-4 gap-xs">
        {/* Pinned items first */}
        {!isCollapsed && pinnedItems.size > 0 && (
          <div className="mb-4">
            <div className="px-sm py-sm text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              Pinned
            </div>
            {filteredNavigation
      ...
      <div className="border-t border-border p-sm">
        ...
        >
          ...
          {isCollapsed && (
            <div className="absolute left-full ml-2 px-sm py-xs bg-popover text-popover-foreground text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
              Profile
            </div>
          )}
        </button>
