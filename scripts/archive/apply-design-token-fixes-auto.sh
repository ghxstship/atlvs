...
# GHXSTSHIP Zero Tolerance Design Token Fix Application (AUTO)
...
apply_fix() {
    ...
    # Apply fix - process files one by one
    find apps/web -type f \( -name "*.tsx" -o -name "*.ts" \) -print0 2>/dev/null | while IFS= read -r -d '' file; do
