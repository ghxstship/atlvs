  ...
  ({ className, title, subtitle, action, children, ...props }, ref) => {
    return (
      <div
        ...
        className={twMerge(
          clsx(
            'flex items-start justify-between space-y-xs.5 pb-md border-b border-thin border-subtle',
            className
          )
        )}
        {...props}
      >
        ...
        {action && (
          <div className="flex items-center space-x-sm ml-md">
            {action}
          </div>
        )}
      </div>
...
export const CardContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={twMerge(clsx('pt-md font-body', className))}
        {...props}
      />
    );
  }
  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ...
        className={twMerge(
          clsx(
            'flex items-center justify-between pt-md mt-md border-t border-thin border-subtle font-body',
            className
          )
        )}
        {...props}
      ...
      >
        {badge && (
          <div className="absolute top-4 right-4">
            <span className="inline-flex items-center rounded-full bg-primary/10 px-sm py-xs text-xs font-medium text-primary">
              {badge}
            </span>
          </div>
        )}
