...
const badgeVariants = cva(
  'inline-flex items-center rounded-full border px-sm.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
  {
    variants: {
      ...
      size: {
        xs: 'text-xs px-xs.5 py-0.5 h-5',
        sm: 'text-xs px-sm py-0.5 h-6',
        md: 'text-sm px-sm.5 py-0.5 h-7',
        lg: 'text-sm px-sm py-xs h-8',
      },
    },
    defaultVariants: {
      variant: 'default',
  ...
  ({ className, variant, size, dot, removable, onRemove, children, ...props }, ref) => (
    ...
    >
      ...
      {removable && onRemove && (
        <button
          onClick={onRemove}
          className="ml-xs hover:bg-muted rounded-full p-0.5 transition-colors"
          type="button"
        >
