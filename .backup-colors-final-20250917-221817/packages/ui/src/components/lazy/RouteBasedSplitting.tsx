  ...
  if (!config) {
    return (
      <Card className="p-xl border-destructive bg-destructive/10">
        <h3 className="text-lg font-semibold text-destructive mb-sm">Route Not Found</h3>
        <p className="text-destructive">Route "{routeKey}" is not configured.</p>
      </Card>
    );
  }
      ...
      {Object.entries(routeConfigs).map(([key, config]) => (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-muted-foreground/60">{config.description}</div>
          </div>
        </button>
      ))}
    </nav>
...
export const BundleSizeMonitor: React.FC = () => {
  ...
  return (
    <Card className="p-md mb-md bg-warning/10 border-warning">
      <h4 className="font-semibold text-warning mb-sm">Bundle Monitor (Dev Only)</h4>
      <div className="text-sm text-warning space-y-xs">
