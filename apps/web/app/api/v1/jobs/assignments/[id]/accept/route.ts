...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  return Sentry.startSpan({ name: 'assignments.accept' }, async () => {
    ...
    try {
      const accepted = await services.jobs.acceptAssignment(ctx, params.id);
      return NextResponse.json({ assignment: accepted }, { status: 200 });
    } catch (error: any) {
