...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  return Sentry.startSpan({ name: 'contracts.activate' }, async () => {
    ...
    try {
      const activated = await services.jobs.activateContract(ctx, params.id);
      return NextResponse.json({ contract: activated }, { status: 200 });
    } catch (error: any) {
