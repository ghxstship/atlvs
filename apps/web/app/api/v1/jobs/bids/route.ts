...
const CreateBidSchema = z.object({
  ...
  type: z.enum(['fixed_price', 'time_materials', 'cost_plus', 'unit_price']),
...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'bids.list' }, async () => {
    ...
    const items = await services.jobs.listBids(ctx, filters);
    return NextResponse.json({ items }, { status: 200 });
  });
}

export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'bids.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const { data } = await sb.auth.getUser();
    const userId = data.user?.id;
    const ctx = getTenantContextFromRequest(request, userId);
    ...
    try {
      ...
      const created = await services.jobs.createBid(ctx, input);
