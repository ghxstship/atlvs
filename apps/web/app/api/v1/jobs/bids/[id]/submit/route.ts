...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  return Sentry.startSpan({ name: 'bids.submit' }, async () => {
    ...
    try {
      const submitted = await services.jobs.submitBid(ctx, params.id);
      return NextResponse.json({ bid: submitted }, { status: 200 });
    } catch (error: any) {
