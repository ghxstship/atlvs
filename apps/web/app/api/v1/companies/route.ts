...
import { getI18nPrefs, formatDateTime } from '../../../../lib/i18n';
...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'companies.list' }, async () => {
    ...
    const items = await services.companies.list(ctx, { limit, offset });
    const prefs = getI18nPrefs(request);
    const itemsWithDisplay = items.map((c: any) => ({
      ...c,
      display: {
        createdAtFormatted: c.createdAt ? formatDateTime(c.createdAt, prefs.timeZone, prefs.locale) : null,
        updatedAtFormatted: c.updatedAt ? formatDateTime(c.updatedAt, prefs.timeZone, prefs.locale) : null
      }
    }));
...
export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'companies.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const ci = getCiBypassContext(request);
    const ctx = ci ?? getTenantContextFromRequest(request, (await sb.auth.getUser()).data.user?.id);
    ...
    const created = await services.companies.create(ctx, input as any);
    const prefs = getI18nPrefs(request);
    const companyWithDisplay = {
      ...created,
      display: {
        createdAtFormatted: created?.createdAt ? formatDateTime(created.createdAt, prefs.timeZone, prefs.locale) : null,
        updatedAtFormatted: created?.updatedAt ? formatDateTime(created.updatedAt, prefs.timeZone, prefs.locale) : null
      }
    };
    return NextResponse.json({ company: companyWithDisplay }, { status: 201 });
  });
}
