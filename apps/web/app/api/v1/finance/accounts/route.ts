...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'finance.accounts.list' }, async () => {
    ...
    const items = await services.finance.listAccounts(ctx, kind, { limit, offset });
    const prefs = getI18nPrefs(request);
    const itemsWithDisplay = items.map((account: any) => ({
      ...account,
      display: {
        balanceFormatted: formatCurrency(Number(account.balance), account.currency, prefs.locale),
        createdAtFormatted: account.createdAt ? formatDateTime(account.createdAt, prefs.timeZone, prefs.locale) : null,
        lastReconciledAtFormatted: account.lastReconciledAt ? formatDateTime(account.lastReconciledAt, prefs.timeZone, prefs.locale) : null
      }
    }));
...
export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'finance.accounts.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const ci = getCiBypassContext(request);
    const ctx = ci ?? getTenantContextFromRequest(request, (await sb.auth.getUser()).data.user?.id);
    ...
    const input = {
      ...
      routingNumber: body?.routingNumber ? String(body.routingNumber) : undefined,
      currency: String(body?.currency ?? 'USD'),
      description: body?.description ? String(body.description) : undefined
    };

    const created = await services.finance.createAccount(ctx, input as any);
