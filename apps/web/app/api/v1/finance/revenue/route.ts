...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'finance.revenue.list' }, async () => {
    ...
    const items = await services.finance.listRevenue(ctx, filters, { limit, offset });
    const prefs = getI18nPrefs(request);
    const itemsWithDisplay = items.map((revenue: any) => ({
      ...revenue,
      display: {
        amountFormatted: formatCurrency(Number(revenue.amount), revenue.currency, prefs.locale),
        expectedDateFormatted: revenue.expectedDate ? formatDateTime(revenue.expectedDate, prefs.timeZone, prefs.locale) : null,
        receivedDateFormatted: revenue.receivedDate ? formatDateTime(revenue.receivedDate, prefs.timeZone, prefs.locale) : null,
        createdAtFormatted: revenue.createdAt ? formatDateTime(revenue.createdAt, prefs.timeZone, prefs.locale) : null
      }
    }));
...
export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'finance.revenue.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const ci = getCiBypassContext(request);
    const ctx = ci ?? getTenantContextFromRequest(request, (await sb.auth.getUser()).data.user?.id);
    ...
    const input = {
      ...
      clientCompanyId: body?.clientCompanyId ? String(body.clientCompanyId) : undefined,
      contractReference: body?.contractReference ? String(body.contractReference) : undefined
    };

    const created = await services.finance.createRevenue(ctx, input as any);
