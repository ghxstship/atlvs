...
export async function GET(request: NextRequest) {
  try {
    ...
    const analytics = await fetchCertificationAnalytics(supabase, orgId, targetUserId, filters);

    return NextResponse.json(analytics);
  } catch (error) {
    console.error('Error in GET /api/v1/profile/certifications/analytics:', error);
