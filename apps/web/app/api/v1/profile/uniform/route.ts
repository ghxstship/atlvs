...
import {
  uniformSizingFilterSchema,
  uniformSizingUpdateSchema,
  bulkActionSchema,
  exportSchema,
  fetchUniformSizing,
  fetchUniformSizings,
  updateUniformSizing,
  fetchUniformSizingStats,
  performBulkAction,
  exportUniformSizings,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (statsMode) {
      // Get uniform sizing statistics
      const stats = await fetchUniformSizingStats(supabase, orgId);
      return NextResponse.json(stats);
    }

    if (listMode) {
      // List multiple uniform sizings
      const filters = uniformSizingFilterSchema.parse(Object.fromEntries(url.searchParams));
      const result = await fetchUniformSizings(supabase, orgId, filters);
      return NextResponse.json(result);
    }

    if (targetUserId) {
      // Get specific uniform sizing
      const canViewOthers = hasPermission(membership.role, 'admin');
      if (targetUserId !== user!.id && !canViewOthers) {
        return NextResponse.json({ error: 'Access denied' }, { status: 403 });
      }

      const sizing = await fetchUniformSizing(supabase, orgId, targetUserId);
      if (!sizing) {
        return NextResponse.json({ error: 'Uniform sizing not found' }, { status: 404 });
      }

      return NextResponse.json(sizing);
    }

    // Default: get current user's uniform sizing
    const sizing = await fetchUniformSizing(supabase, orgId, user!.id);
    if (!sizing) {
      return NextResponse.json({ error: 'Uniform sizing not found' }, { status: 404 });
    }
    ...
    console.error('Error in GET /api/v1/profile/uniform:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Check if user can update the target user's uniform sizing
    const canUpdateOthers = hasPermission(membership.role, 'admin');
    if (targetUserId !== user!.id && !canUpdateOthers) {
      return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    }

    const sizingData = uniformSizingUpdateSchema.parse(body);
    const updatedSizing = await updateUniformSizing(
      supabase,
    ...
    if (!updatedSizing) {
      return NextResponse.json({ error: 'Failed to update uniform sizing' }, { status: 500 });
    }

    return NextResponse.json(updatedSizing);
  } catch (error) {
    console.error('Error in PUT /api/v1/profile/uniform:', error);
    if (error instanceof Error) {
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  ...
  try {
    ...
    switch (action) {
      ...
      case 'export': {
        ...
        const result = await exportUniformSizings(
          supabase,
          membership.organization_id,
          exportConfig
        );
        ...
        headers.set('Content-Disposition', `attachment; filename="uniform-sizings.${exportConfig.format}"`);
    ...
    console.error('Error in POST /api/v1/profile/uniform:', error);
    if (error instanceof Error) {
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
