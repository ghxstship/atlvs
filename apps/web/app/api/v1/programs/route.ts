...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'programs.list' }, async () => {
    ...
    const items = await services.programs.list(ctx, { limit, offset });
    return NextResponse.json({ items }, { status: 200 });
  });
}

export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'programs.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const ci = getCiBypassContext(request);
    const ctx = ci ?? getTenantContextFromRequest(request, (await sb.auth.getUser()).data.user?.id);
    ...
    const created = await services.programs.create(ctx, input as any);
