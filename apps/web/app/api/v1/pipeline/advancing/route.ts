...
const AdvancingItemSchema = z.object({
...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'pipeline.advancing.list' }, async () => {
    ...
    const items = await services.pipeline.listAdvancingItems(ctx, projectId);
    return NextResponse.json({ items }, { status: 200 });
  });
}

export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'pipeline.advancing.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const ci = getCiBypassContext(request);
    const ctx = ci ?? getTenantContextFromRequest(request, (await sb.auth.getUser()).data.user?.id);
    ...
    const validation = AdvancingItemSchema.safeParse(body);
    ...
    const created = await services.pipeline.createAdvancingItem(ctx, item);
