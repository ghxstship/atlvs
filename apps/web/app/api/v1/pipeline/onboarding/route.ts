...
const OnboardingTaskSchema = z.object({
  ...
  category: z.enum(['documentation', 'training', 'equipment', 'access', 'compliance', 'other']).default('other'),
...
export async function GET(request: NextRequest) {
  return Sentry.startSpan({ name: 'pipeline.onboarding.list' }, async () => {
    ...
    const tasks = await services.pipeline.listOnboardingTasks(ctx);
    return NextResponse.json({ tasks }, { status: 200 });
  });
}

export async function POST(request: NextRequest) {
  return Sentry.startSpan({ name: 'pipeline.onboarding.create' }, async () => {
    const { sb, services } = getSupabaseAndServices();
    const ci = getCiBypassContext(request);
    const ctx = ci ?? getTenantContextFromRequest(request, (await sb.auth.getUser()).data.user?.id);
    ...
    const validation = OnboardingTaskSchema.safeParse(body);
    ...
    const created = await services.pipeline.createOnboardingTask(ctx, task);
