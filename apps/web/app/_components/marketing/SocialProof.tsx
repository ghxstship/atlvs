        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="bg-background p-6 rounded-lg shadow-sm border border-border">
              <Quote className="w-8 h-8 color-muted/50 mb-4" />
              <div className="flex mb-4">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className="w-4 h-4 color-warning fill-current" />
                ))}
              </div>
              <p className="color-foreground mb-4">
                {testimonial.content}
              </p>
              <div>
                <div className="text-heading-4 color-foreground">{testimonial.name}</div>
