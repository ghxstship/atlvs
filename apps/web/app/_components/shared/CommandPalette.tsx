...
export function CommandPalette({ navSections }: { navSections: NavSection[] }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Command Palette" className="fixed inset-none z-50 flex items-start justify-center p-md">
      <div className="absolute inset-none bg-background/80 backdrop-blur-sm" role="button" tabIndex={0} onClick={() => setOpen(false)} />
      <div className="relative w-full max-w-xl rounded-xl bg-card shadow-elevation-2xl border border-border overflow-hidden">
        ...
        <div className="max-h-lg overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="p-md text-body-sm color-muted">No results</div>
          ) : (
            <ul className="p-sm">
              {filtered.map((cmd, index) => (
                <li key={cmd.id}>
                  <button aria-label="button"
                    className="w-full text-left px-md py-sm rounded-md hover:bg-secondary focus:outline-none focus:ring-primary ring-primary focus:ring-primary focus:ring-primary focus:ring-ring"
                    type="button"
                    onClick={() => onSelect(cmd)}
                  >
                    <div className="text-body-sm">{cmd.label}</div>
                    <div className="text-body-sm color-muted">{cmd.href}</div>
                  </button>
                </li>
              ))}
            </ul>
