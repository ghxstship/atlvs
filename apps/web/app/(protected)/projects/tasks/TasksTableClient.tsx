...
export default function TasksTableClient({ rows, orgId }: { rows: TaskRow[]; orgId: string }) {
  ...
  const form = useForm<z.infer<typeof schema>>({
    ...
    values: current ? { title: current.title, status: current.status || 'todo', due_at: current.due_at ?? null } : { title: '', status: 'todo', due_at: null },
    mode: 'onChange'
  });
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="sticky top-0">
            ...
            <th className="border-b p-2 text-left">{t('grid.dueAt')}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="hover:bg-accent/20 cursor-pointer" onClick={() => { setOpenId(r.id); setTab('details'); }}>
              ...
              <td className="border-b p-2">{r.due_at ? new Date(r.due_at).toLocaleDateString() : '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      >
        ...
        {tab === 'comments' && (
          <div className="space-y-3">
            <form action={addComment} className="flex items-start gap-2" aria-label={t('drawer.comments')}>
