        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="hover:bg-accent/20 cursor-pointer" onClick={() => { setOpenId(r.id); setTab('details'); }}>
              ...
              <td className="border-b p-2">{r.starts_at ? new Date(r.starts_at).toLocaleDateString() : '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      >
        ...
        {tab === 'comments' && (
          <div className="space-y-3">
            <form action={addComment} className="flex items-start gap-2" aria-label="Add comment">
