...
export default function ScheduleClient({ data, orgId }: { data: ScheduleData; orgId: string }) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active': case 'in_progress': case 'todo': return 'bg-primary/10 color-primary';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded border px-3 py-1 text-body-sm"
          >
            ...
            {filteredData.projects.map(project => (
              <div key={project.id} className="border rounded-lg p-4">
                ...
                {project.starts_at && project.ends_at && (
                  <div className="flex items-center gap-4 text-body-sm color-muted mb-3">
                ...
                <div className="space-y-2">
                  {filteredData.milestones
                    ...
                    .map(milestone => (
                      <div key={milestone.id} className="flex items-center gap-3 p-2 bg-secondary/50 rounded">
      ...
      {view === 'calendar' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-body-sm form-label color-muted mb-2">
            <div>Sun</div>
            <div>Mon</div>
            <div>Tue</div>
            <div>Wed</div>
                ...
                >
                  ...
                  <div className="space-y-1">
                    {dayItems.slice(0, 2).map((item, idx) => (
                      <div
                        key={idx}
                        className={`text-body-sm p-1 rounded truncate ${
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {upcomingItems.map((item, index) => (
              <div key={index} className="flex items-center gap-4 p-3 border rounded-lg hover:bg-secondary/50">
                <div className="flex items-center gap-2">
                  {item.type === 'milestone' ? (
                    <CheckCircle className="w-4 h-4 color-primary" />
                  ) : (
