...
const createLineupSchema = z.object({
 ...
 performer_name: z.string().min(1, "Performer name is required").max(255),
...
type CreateProgrammingLineupDrawerProps = {
 open: boolean;
...
export default function CreateProgrammingLineupDrawer({
 open,
 onOpenChange,
 orgId,
 currentUserId,
 ...
 const watchedCurrency = watch("contract_details.currency");
 const watchedPerformerType = watch("performer_type");

 const onSubmit = async (data: CreateLineupFormData) => {
 try {
 setLoading(true);
 ...
 {errors.event_id && (
 <p className="text-sm text-destructive mt-1">{errors.event_id.message}</p>
 )}
 </div>
