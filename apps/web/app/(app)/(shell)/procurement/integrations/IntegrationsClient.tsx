...
const Switch = ({ checked, onCheckedChange }: { checked: boolean; onCheckedChange: (checked: boolean) => void }) => (
 <button
 type="button"
 className={`relative inline-flex h-icon-md w-11 items-center rounded-full transition-colors ${
 checked ? 'bg-blue-600' : 'bg-gray-200'
 }`}
 onClick={() => onCheckedChange(!checked)}
 >
 <span
 className={`inline-block h-icon-xs w-icon-xs transform rounded-full bg-white transition-transform ${
...
export default function IntegrationsClient({ className, orgId }: IntegrationsClientProps) {
 ...
 status: 'connected',
 provider: 'SAP',
 icon: <Database className="h-icon-md w-icon-md" />,
 ...
 status: 'connected',
 provider: 'Amazon',
 icon: <ShoppingCart className="h-icon-md w-icon-md" />,
 ...
 status: 'connected',
 provider: 'Stripe',
 icon: <CreditCard className="h-icon-md w-icon-md" />,
 ...
 case 'disconnected':
 return <XCircle className="h-icon-xs w-icon-xs text-gray-500" />;
 case 'error':
 return <AlertCircle className="h-icon-xs w-icon-xs text-red-500" />;
 case 'pending':
 return <AlertCircle className="h-icon-xs w-icon-xs text-yellow-500" />;
 default:
 return <XCircle className="h-icon-xs w-icon-xs text-gray-500" />;
 }
 };
 ...
 switch (category) {
 case 'erp':
 return <Database className="h-icon-xs w-icon-xs" />;
 case 'marketplace':
 return <ShoppingCart className="h-icon-xs w-icon-xs" />;
 case 'payment':
 return <CreditCard className="h-icon-xs w-icon-xs" />;
 case 'shipping':
 return <Truck className="h-icon-xs w-icon-xs" />;
 case 'analytics':
 return <BarChart3 className="h-icon-xs w-icon-xs" />;
 case 'accounting':
 return <FileText className="h-icon-xs w-icon-xs" />;
 case 'inventory':
 return <Database className="h-icon-xs w-icon-xs" />;
 default:
 return <Globe className="h-icon-xs w-icon-xs" />;
 }
 };
 ...
 <Zap className="h-icon-xs w-icon-xs text-muted-foreground" />
 </CardHeader>
 <CardContent>
 ...
 <Cloud className="h-icon-xs w-icon-xs text-muted-foreground" />
 </CardHeader>
 <CardContent>
 ...
 {/* Integrations Tabs */}
 <Tabs defaultValue="all-integrations" className="space-y-md">
 <TabsList>
 <TabsTrigger value="all-integrations">
 <Globe className="h-icon-xs w-icon-xs mr-2" />
 All Integrations
 </TabsTrigger>
 ...
 <ShoppingCart className="h-icon-xs w-icon-xs mr-2" />
 Marketplace
 </TabsTrigger>
 <TabsTrigger value="webhooks">
 <Webhook className="h-icon-xs w-icon-xs mr-2" />
 Webhooks
 </TabsTrigger>
 </TabsList>

 <TabsContent value="all-integrations" className="space-y-md">
 ...
 <div className="p-xs bg-gray-100 rounded-lg">
 {integration.icon}
 </div>
 <div>
 ...
 <TabsContent value="active" className="space-y-md">
 ...
 <TabsContent value="marketplace" className="space-y-md">
 <Card>
 <CardHeader>
 ...
 .map((integration) => (
 <div key={integration.id} className="border rounded-lg p-md">
 ...
 <TabsContent value="webhooks" className="space-y-md">
 <Card>
 <CardHeader>
 ...
 <div className="grid grid-cols-2 md:grid-cols-3 gap-xs mt-2">
 {[
 'request.created',
 'request.approved',
 'order.placed',
 ...
 <label key={event} className="flex items-center space-x-xs">
