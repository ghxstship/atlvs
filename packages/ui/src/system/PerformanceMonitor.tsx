  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 transition-all ${isMinimized ? 'w-48' : 'w-96'}`}>
      <Card className="shadow-2xl border-2">
        <CardHeader className="p-4 cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
        ...
        {!isMinimized && (
          <CardContent className="p-4 pt-0">
            ...
            <div className="flex gap-2 mb-4">
              <button
                className={`px-3 py-1 text-xs rounded ${selectedTab === 'vitals' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
                onClick={() => setSelectedTab('vitals')}
              >
                Web Vitals
              </button>
              <button
                className={`px-3 py-1 text-xs rounded ${selectedTab === 'network' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
                onClick={() => setSelectedTab('network')}
              >
                Network
              </button>
              <button
                className={`px-3 py-1 text-xs rounded ${selectedTab === 'components' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
