...
export function DatabaseTransactionManager({
  ...
  return (
    <div className={`bg-background border border-border rounded-lg p-md ${className}`}>
        ...
        <div className="flex items-center gap-sm">
          ...
          {currentBatch && (
            <Badge variant={currentBatch.status === 'completed' ? 'default' : 'secondary'}>
              ...
              <span className="ml-xs">{currentBatch.operations.length} operations</span>
            </Badge>
          )}
        </div>
      </div>
      ...
      {/* Current Transaction */}
      {currentBatch && (
        <div className="mb-md p-sm bg-muted rounded-md">
          ...
          <div className="flex justify-end gap-sm mt-sm">
            {currentBatch.status === 'pending' && (
              <>
                ...
                >
                  {isExecuting ? (
                    <Loader2 className="h-3 w-3 animate-spin mr-xs" />
                  ) : (
                    <Save className="h-3 w-3 mr-xs" />
                  )}
                  Execute Transaction
                </Button>
              </>
            ...
            {currentBatch.status === 'completed' && (
              ...
              >
                <RotateCcw className="h-3 w-3 mr-xs" />
                Rollback
              </Button>
            )}
          </div>
      ...
      {transactionHistory.length > 0 && (
        <div>
          <h4 className="text-sm font-medium mb-sm">Recent Transactions</h4>
          <div className="space-y-sm max-h-40 overflow-y-auto">
            {transactionHistory.slice(-5).reverse().map(batch => (
              <div key={batch.id} className="flex items-center justify-between text-sm p-sm bg-muted rounded">
                <div className="flex items-center gap-sm">
