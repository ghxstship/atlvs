...
const sidebarVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      variant: {
        default: '',
        floating: 'mx-sm my-sm rounded-lg border border-border shadow-surface bg-background/95 backdrop-blur',
        overlay: 'fixed inset-y-0 left-0 z-50 shadow-modal',
      },
    },
    defaultVariants: {
      state: 'expanded',
...
const navItemVariants = cva(
  [
    'flex items-center gap-sm px-sm py-sm mx-sm rounded-lg text-sm font-medium',
...
export const SidebarNavigation: React.FC<SidebarNavigationProps> = ({
    ...
    return (
      <div key={item.id} className="relative">
        ...
        >
          ...
          {isCollapsed && (
            <div className="absolute left-full ml-sm px-sm py-xs bg-popover text-popover-foreground text-xs rounded opacity-0 group-hover:opacity-100 motion-safe:transition-opacity motion-safe:duration-200 pointer-events-none whitespace-nowrap z-50">
              {item.label}
        ...
        {hasChildren && isExpanded && !isCollapsed && (
          <div id={childContainerId} className="ml-sm border-l border-border animate-in slide-in-from-top-2 motion-safe:duration-200">
  ...
  if (isMobile && variant === 'overlay') {
    return (
      <>
        ...
        <button
          className="lg:hidden fixed top-4 left-4 z-50 p-sm bg-background border border-border rounded-lg shadow-surface"
        ...
        {isMobileOpen && (
          ...
          >
            ...
            <div className={sidebarVariants({ state: 'expanded', variant: 'overlay' })}>
              ...
              {/* Navigation */}
              <nav className="flex-1 overflow-y-auto px-sm pb-md">
  ...
  return (
    <div aria-label="Sidebar Navigation" className={twMerge(sidebarVariants({ 
      ...
      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto px-sm pb-md gap-xs">
        {/* Pinned items first */}
      ...
      <div className="border-t border-border p-sm">
        ...
        >
          ...
          {isCollapsed && (
            <div className="absolute left-full ml-sm px-sm py-xs bg-popover text-popover-foreground text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
              Profile
            </div>
          )}
        </button>
