...
const inputVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      size: {
        sm: ['h-icon-lg', 'px-2', 'text-xs'],
        default: ['h-icon-xl', 'px-3'],
  ...
  (
    ...
    const inputElement = (
      <div className="relative">
        ...
        {leftIcon && !leftAddon && (
          <div className="absolute left-0 top-0 bottom-0 flex items-center pl-3 pointer-events-none">
            <span className="text-muted-foreground h-icon-xs w-icon-xs">{leftIcon}</span>
          </div>
        )}
        ...
        {loading && (
          <div className="absolute right-0 top-0 bottom-0 flex items-center pr-3">
            <svg
              className="h-icon-xs w-icon-xs animate-spin text-muted-foreground"
        ...
        {!loading && (rightIcon || rightAddon) && (
          <div className="absolute right-0 top-0 bottom-0 flex items-center pr-3 pointer-events-none">
            {rightAddon ? (
              ...
              <span className="text-muted-foreground h-icon-xs w-icon-xs">{rightIcon}</span>
            )}
          </div>
        )}
      </div>
...
const PasswordInput = React.forwardRef<HTMLInputElement, InputProps>(
  ({ rightIcon, ...props }, ref) => {
    ...
    const toggleIcon = (
      ...
      >
        {showPassword ? (
          <svg className="h-icon-xs w-icon-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24">
