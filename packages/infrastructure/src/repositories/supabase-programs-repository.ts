...
export class SupabaseProgramsRepository implements ProgramRepository {
  ...
  async findById(id: string, orgId: string): Promise<Program | null> {
    return Sentry.startSpan({ name: 'repo.programs.findById' }, async () => {
  ...
  async list(orgId: string, limit = 20, offset = 0): Promise<Program[]> {
    return Sentry.startSpan({ name: 'repo.programs.list' }, async () => {
  ...
  async create(entity: Program): Promise<Program> {
    return Sentry.startSpan({ name: 'repo.programs.create' }, async () => {
      const row = {
        id: entity.id,
  ...
  async update(id: string, partial: Partial<Program>): Promise<Program> {
    return Sentry.startSpan({ name: 'repo.programs.update' }, async () => {
      const patch: any = {};
  ...
  async delete(id: string): Promise<void> {
    return Sentry.startSpan({ name: 'repo.programs.delete' }, async () => {
